...
def generate_loss_surface(
    ...
    # load the model
    raw_data = pickle.load(open(os.path.join(MODEL_PATH, "noise_perf_comp_uci_iris_2026-06-05_noise_0.100.pkl"), "rb"))
...
def main():
    ...
    alphas_ = jnp.linspace(-1.0, 1.0, 101)
    betas_ = jnp.linspace(-1.0, 1.0, 101)
    ...
    ax.set_ylabel('Beta')
    ax.set_zlabel('Loss')
    plt.savefig("../plots/tmp_loss_surface.png")
    plt.show()
