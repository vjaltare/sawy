...
def parse_args():
    ...
    parser.add_argument("--thresholds", nargs="+", type=float, default=[-0.1, 0.1])
...
class ResNet(nnx.Module):
    ...
    def __call__(self, x):
        ...
        x = self.linear1(x)
        x = self.activation_function(x, key=self.rngs.next()) # pass a new key for every activation call
        x = self.linear2(x)
        x = self.activation_function(x, key=self.rngs.next())
        x = self.classifier(x)
        return x
